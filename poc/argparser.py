...
def do_split(text:str, separator:str=" ", open_splits:list=None, close_splits:list=None, special_separators:list=None):
    ...
    while index < len(text):
        ...
        if l_2 in open_splits and (l_1 == separator or separator == ""):
            ...
            segment = do_encapsulated_split(text[index:], sep_init=l_2, sep_end=close_splits[open_split_index], include_separators=l_2 in special_separators)
            segments.append(segment)
            index += len(segment) + 2
            segment = ""
        elif l_2 == separator:
...
def do_encapsulated_split(text:str, sep_init="(", sep_end=")", include_separators=False):
        ...
        if operand in operators:
            ...
            if len(previous_oper) > 1:
                previous_oper = retrieve_ops2_tree(previous_oper)
                    ...
                    previous_oper = retrieve_ops_tree(do_split(previous_oper[0].strip(), open_splits=open_splits, close_splits=close_splits))
...
def to_mongo_query_list(ops_list:list):
    ...
    if operation == "eq":
        ...
        result[operand1] = {"$in": do_split(operand2[1:-1], separator="", open_splits=["'"], close_splits=["'"])}
...
def to_mongo_query_dict(ops_dict):
    ...
    if len(base) > 0:
        result = to_mongo_query_list(base[0])
...
splits = do_split("value.hi = 23", open_splits=["(", "[", "'"], close_splits=[")", "]", "'"], special_separators="[")
print(splits)
splits = do_split("value.hi in ['hi', 'he', 'ho']", open_splits=["(", "[", "'"], close_splits=[")", "]", "'"], special_separators="[")
print(splits)
ops = retrieve_ops_tree(splits, open_splits=["(", "[", "'"], close_splits=[")", "[", "'"])
print(ops)
mongo = to_mongo_query_dict(ops)
...
print(do_split("'a', 'b', 'c, d'", separator="", open_splits=["'"], close_splits=["'"]))
